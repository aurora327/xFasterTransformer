...
if __name__ == "__main__":
    ...
    if model.rank == 0:
        ...
        print("\n")
        print("=" * 40 + args.model_name + " Final Performance" + "=" * 40)
        print(f"Inference Avg Latency:\t{np.mean(total_times):.2f} ms")
        print(f"1st token Avg Latency:\t{np.mean(first_token_times):.2f} ms")
        print(f"2nd token Max Latency:\t{np.max(next_token_times):.2f} ms")
        print(f"2nd token Min Latency:\t{np.min(next_token_times):.2f} ms")
        print(f"2nd token P90 Latency:\t{np.percentile(next_token_times, 90):.2f} ms")
        print(f"2nd token Avg Latency:\t{np.mean(next_token_times):.2f} ms")
        print(f"2nd token Latency:\t{np.percentile(next_token_times, 90):.2f} ms")
        print(f"Throughput--(w/o 1st):\t{1000 / np.percentile(next_token_times, 90) * args.batch_size:.2f} tokens/s")
        print("=" * 120, "\n" * 3)
    else:
